...
test("full red bar returns 1.0", function()
    ...
    local val = core.sample_bar(data, stride, {x = 0, y = 0, w = 20, h = 2, active_r = 255, active_g = 0, active_b = 0, color_dist = 60})
    eq(val, 1.0, "full bar")
end)
...
test("blue channel detection", function()
    ...
    local val = core.sample_bar(data, stride, {x = 0, y = 0, w = 10, h = 1, active_r = 0, active_g = 0, active_b = 200, color_dist = 60})
    eq(val, 1.0, "full blue")
end)

test("green channel detection", function()
    local data, stride = make_image(10, 1, function() return 0, 200, 0 end)
    local val = core.sample_bar(data, stride, {x = 0, y = 0, w = 10, h = 1, active_r = 0, active_g = 200, active_b = 0, color_dist = 60})
    eq(val, 1.0, "full green")
end)

test("rejects similar but darker shade", function()
    -- Active color is bright red (200,0,0), dark red (100,20,20) should NOT match
    local data, stride = make_image(10, 1, function() return 100, 20, 20 end)
    local val = core.sample_bar(data, stride, {x = 0, y = 0, w = 10, h = 1, active_r = 200, active_g = 0, active_b = 0, color_dist = 60})
    eq(val, 0, "dark red rejected")
end)

test("rejects gray", function()
    local data, stride = make_image(10, 1, function() return 128, 128, 128 end)
    local val = core.sample_bar(data, stride, {x = 0, y = 0, w = 10, h = 1, active_r = 200, active_g = 0, active_b = 0, color_dist = 60})
    eq(val, 0, "gray rejected")
end)

test("color_dist_sq", function()
    eq(core.color_dist_sq(0,0,0, 0,0,0), 0, "same")
    eq(core.color_dist_sq(255,0,0, 0,0,0), 255*255, "red vs black")
    eq(core.color_dist_sq(100,0,0, 200,0,0), 10000, "100 apart")
end)
...
local DIGIT_PATTERNS = core.DIGIT_PATTERNS
